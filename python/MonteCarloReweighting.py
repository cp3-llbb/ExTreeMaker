...
class MonteCarloReWeighting:
    ...
    def _eventWeight(self, fwevent=None, mode="none"):
        ...
        for particle in fwevent.genParticles:
            if particle.pdgId() == 23:
                theZpt = particle.pt()
                break
        bparts = [part for part in fwevent.genParticles if
                  (499 < math.fabs(part.pdgId()) < 600) or (4999 < math.fabs(part.pdgId()) < 6000)]
        ...
        if math.fabs(maxbjetPt - 25) < 10:
            bjetptweight = math.exp(-1 * 0.015 * 25) * math.exp(
                0.015 * maxbjetPt)  # that's a 25% variation of the nominal slope: 0.059
        else:
            bjetptweight = 1
        ...
        if "Merging" in mode:
            ...
            sfs_Zbx = 1.14
            sfs_Zxx = 1.24
        sfs_dy = sfs_Zbb * MonteCarloSelection.isRecoZbbEvent(
            fwevent.dijet_all) + sfs_Zbx * MonteCarloSelection.isRecoZbEvent(fwevent.dijet_all) + sfs_Zxx * (
        not MonteCarloSelection.isRecoZbbEvent(fwevent.dijet_all) and not MonteCarloSelection.isRecoZbEvent(
            fwevent.dijet_all))
            ...
            for htbin in [200, 400, float("inf")]:
                ...
                for B in self.bins:
                    if (("ll_pt<" + str(ptbin) in B and "HT<" + str(htbin) in B)
                        or (("ll_pt<" + str(ptbin) in B and htbin == float("inf") and not "HT<" in B) or (
                                    ptbin == float("inf") and "HT<" + str(htbin) in B and not "ll_pt<" in B))
                        or (ptbin == float("inf") and htbin == float(
                            "inf") and not "HT<" in B and not "ll_pt<" in B and ">=" in B)):
    ...
    def _is_final_bhad(self, genpart):
        ...
        if len([genpart.daughter(i) for i in range(genpart.numberOfDaughters()) if
                MonteCarloSelection.is_Bhad(genpart.daughter(i))]): return False
        return True
