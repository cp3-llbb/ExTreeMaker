...
class BtaggingWeight:
    ...
    def __init__(self, jmin1, jmax1, jmin2, jmax2, file, btagging="CSV", WP=["M", "L"]):
        ...
        self.WP = WP
        if "L" in WP or "HE" in WP:
            self.algo1 = 1
        else:
            self.algo1 = 2
            self.algo2 = 3
        if self.algo1 == 1:
            if "M" in WP or "HP" in WP:
                self.algo2 = 2
            else:
                self.algo2 = 3
    ...
    def setMode(self, mode):
        ...
        if mode == self.WP[1]:
            self.engine.setLimits(1, 999, 0, 999)  # L
        elif mode == self.WP[0]:
            self.engine.setLimits(1, 999, 1, 999)  # M
        elif mode == self.WP[1] + "excl":
            self.engine.setLimits(1, 1, 0, 1)  # L exclusive
        elif mode == self.WP[0] + "excl":
            self.engine.setLimits(1, 999, 1, 1)  # M exclusive
        elif mode == self.WP[1] + self.WP[1]:
            self.engine.setLimits(2, 999, 0, 999)  # LL
        elif mode == self.WP[1] + self.WP[0]:
            self.engine.setLimits(2, 999, 1, 999)  # LM
        elif mode == self.WP[0] + self.WP[0]:
            self.engine.setLimits(2, 999, 2, 999)  # MM

        # Added for inclusive search
        elif mode == self.WP[1] + self.WP[1] + "excl":
            self.engine.setLimits(2, 2, 0, 999)  # LL exclusive
        elif mode == self.WP[0] + self.WP[0] + "excl":
            self.engine.setLimits(2, 999, 2, 2)  # MM exclusive
        elif mode == self.WP[1] + self.WP[1] + self.WP[1]:
            self.engine.setLimits(3, 999, 0, 999)  # LLL
        elif mode == self.WP[0] + self.WP[0] + self.WP[0]:
            self.engine.setLimits(3, 999, 3, 999)  # MMM
        elif mode == self.WP[1] + self.WP[1] + self.WP[1] + "excl":
            self.engine.setLimits(3, 3, 0, 999)  # LLL exclusive
        elif mode == self.WP[0] + self.WP[0] + self.WP[0] + "excl":
            self.engine.setLimits(3, 3, 3, 3)  # MMM exclusive
        elif mode == "0" + self.WP[1] + "excl":
            self.engine.setLimits(0, 0, 0, 0)  # 0L exclusive
        elif mode == "0" + self.WP[0] + "excl":
            self.engine.setLimits(0, 999, 0, 0)  # 0M exclusive
    ...
    def btaggingJet(self, event, catName, Bmode):
        ...
        if "CA8 or subjets" in catName:
            ...
            if subjetinfo["drj1j2"] >= 0.4:
                jetColl = "sub"
            else:
                jetColl = "fat"
        elif "CA8" in catName:
            jetColl = "fat"
        elif "Subjets" in catName:
            jetColl = "sub"
