...
ourtriggers.emutriggers = list(
    set([item for sublist in [i for i in ourtriggers.emurunMap.values()] for item in sublist]))
ourtriggers.triggers = list(set(ourtriggers.mutriggers) | set(ourtriggers.eltriggers) | set(ourtriggers.emutriggers))
ourtriggers.SingleMutriggers = list(
    set([item for sublist in [i for i in ourtriggers.muSinglerunMap.values()] for item in sublist]))
...
def isTriggerMatchPair(l1, l2, runNumber, lumi_section):
    if l1.isMuon() and l2.isMuon():
        ...
        if runNumber >= 160410 and runNumber < 163269:
            if (l1.triggerObjectMatchesByPath("HLT_DoubleMu6_v*", 0, 0).size() > 0) and (
                l2.triggerObjectMatchesByPath("HLT_DoubleMu6_v*", 0, 0).size() > 0):
                return True
        if runNumber >= 163269 and runNumber < 165121:
            if (l1.triggerObjectMatchesByPath("HLT_DoubleMu7_v*", 0, 0).size() > 0) and (
                l2.triggerObjectMatchesByPath("HLT_DoubleMu7_v*", 0, 0).size() > 0):
                return True
        if runNumber >= 165121 and runNumber < 178420:
            if (l1.triggerObjectMatchesByPath("HLT_Mu13_Mu8_v*", 0, 1).size() > 0) and (
                l2.triggerObjectMatchesByPath("HLT_Mu13_Mu8_v*", 0, 1).size() > 0) and (
                (l1.triggerObjectMatchesByFilter("hltDiMuonL3PreFiltered8").size() > 0) or (
                l1.triggerObjectMatchesByFilter("hltDiMuonL3p5PreFiltered8").size() > 0)) and (
                (l2.triggerObjectMatchesByFilter("hltDiMuonL3PreFiltered8").size() > 0) or (
                l2.triggerObjectMatchesByFilter("hltDiMuonL3p5PreFiltered8").size() > 0)) and (
                l1.triggerObjectMatchesByFilter("hltSingleMu13L3Filtered13").size() > 0):
                return True
        if runNumber >= 178420 and runNumber < 180253:
            if (l1.triggerObjectMatchesByPath("HLT_Mu17_Mu8_v*", 0, 1).size() > 0 and l2.triggerObjectMatchesByPath(
                    "HLT_Mu17_Mu8_v*", 0, 1).size() > 0 and (
                l1.triggerObjectMatchesByFilter("hltDiMuonL3p5PreFiltered8").size() > 0) and (
                l2.triggerObjectMatchesByFilter(
                        "hltDiMuonL3p5PreFiltered8").size() > 0) and l1.triggerObjectMatchesByFilter(
                    "hltSingleMu13L3Filtered17").size() > 0):
                return True
            ### data 2012
        if runNumber >= 180253:
            ...
            if ((l1.triggerObjectMatchesByPath("HLT_Mu17_TkMu8_v*", 0, 1).size() > 0 and l2.triggerObjectMatchesByPath(
                    "HLT_Mu17_TkMu8_*", 0, 1).size() > 0) or (
                    l1.triggerObjectMatchesByPath("HLT_Mu17_Mu8_v*", 0, 1).size() > 0 and l2.triggerObjectMatchesByPath(
                    "HLT_Mu17_Mu8_*", 0, 1).size() > 0)):
    ...
    if l1.isElectron() and l2.isElectron():
        if electron_iswrongPS(l1, runNumber, lumi_section):
            ...
            if runNumber < 165121:
                if (l1.triggerObjectMatchesByPath("HLT_Ele17_CaloIdL_CaloIsoVL_Ele8_CaloIdL_CaloIsoVL_v*", 0,
                                                  0).size() > 0) and (l1.triggerObjectMatchesByFilter(
                        "hltEle17CaloIdIsoEle8CaloIdIsoPixelMatchDoubleFilter").size() > 0) and (
                    l2.triggerObjectMatchesByPath("HLT_Ele17_CaloIdL_CaloIsoVL_Ele8_CaloIdL_CaloIsoVL_v*", 0,
                                                  0).size() > 0) and (
                    l2.triggerObjectMatchesByFilter("hltEle17CaloIdIsoEle8CaloIdIsoPixelMatchDoubleFilter").size() > 0):
                    return True
            if runNumber >= 165121 and runNumber < 190455:
                if ((l1.triggerObjectMatchesByPath(
                        "HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0,
                        1).size() > 0) and (l1.triggerObjectMatchesByFilter(
                        "hltEle17TightIdLooseIsoEle8TightIdLooseIsoTrackIsolDoubleFilter").size()) and (
                    l2.triggerObjectMatchesByPath(
                            "HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*",
                            0, 1).size() > 0) and (l2.triggerObjectMatchesByFilter(
                        "hltEle17TightIdLooseIsoEle8TightIdLooseIsoTrackIsolDoubleFilter").size())) or ((
                                                                                                            l1.triggerObjectMatchesByPath(
            ...
            if runNumber >= 190455:
                ...
                if l1.triggerObjectMatchesByPath(
                        "HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0,
                        1).size() > 0 and l2.triggerObjectMatchesByPath(
                        "HLT_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0,
                        1).size() > 0:
                    # print runNumber
                    # l1DoublePaths = []
    ...
    if (l1.isElectron() and l2.isMuon()) or (l2.isElectron() and l1.isMuon()):
        if runNumber >= 190455:
            if (l1.triggerObjectMatchesByPath("HLT_Mu8_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0,
                                              1).size() > 0 and l2.triggerObjectMatchesByPath(
                    "HLT_Mu8_Ele17_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0, 1).size() > 0) or (
                    l1.triggerObjectMatchesByPath("HLT_Mu17_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0,
                                                  1).size() > 0 and l2.triggerObjectMatchesByPath(
                    "HLT_Mu17_Ele8_CaloIdT_CaloIsoVL_TrkIdVL_TrkIsoVL_v*", 0, 1).size() > 0):
                return True
...
def leptonsFromPV_ptSorted(event):
    muList = [muon for muon in event.muons if
              isGoodMuon(muon, "tight") and isFromVertex_SingleLepton(muon, 0.05, event.vertex)]
    elList = [electron for electron in event.electrons if
              isGoodElectron(electron, "tight") and isFromVertex_SingleLepton(electron, 0.05, event.vertex)]
...
def leptonsFromPV_ptSorted_DRllVetoOnFirstTwo(event, DRll_cut=0.3, ptLeadLep=17.0, ptSubLeadLep=8.0):
    if event.ptSortedLeptons is not None:
        ...
        DRll = ROOT.TLorentzVector(l1.px(), l1.py(), l1.pz(), l1.energy()).DeltaR(
            ROOT.TLorentzVector(l2.px(), l2.py(), l2.pz(), l2.energy()))
...
def findBestDiLeptCandidate(event, muChannel=True, eleChannel=True):
    ...
    if first_lept is None or second_lept is None:
        return None
    else:
...
def findDijetPair(event, btagging="CSV", WP=["M", "L"], muChannel=True, eleChannel=False, prejets=""):
    ...
    if btagging == "CSV":
        btagList = [(jet.bDiscriminator("combinedSecondaryVertexBJetTags"), index) for index, jet in
                    enumerate(getattr(event, jetsLabel)) if goodJets[index]]
    elif btagging == "JP":
        btagList = [(jet.bDiscriminator("jetProbabilityBJetTags"), index) for index, jet in
                    enumerate(getattr(event, jetsLabel)) if goodJets[index]]
...
def fatjets(event, pt=30.):
    ...
    for pruned in prunedjets:
        if pruned.pt() > 2 * pt and pruned.bDiscriminator(
                "combinedSecondaryVertexBJetTags") > prunedCSV and pruned.numberOfDaughters() == 2:
            if isGoodJet(pruned.daughter(0), lepPair=event.bestZcandidate, pt=pt) and isGoodJet(pruned.daughter(1),
                                                                                                lepPair=event.bestZcandidate,
                                                                                                pt=pt):
...
def subjets(event):
    ...
    if len(event.fatjets) == 1:
        ...
        print "Error: more than 1 fat jets selected."
