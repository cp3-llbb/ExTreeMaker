...
class configuration:
    # default I/O
    pythonpath = "cp3-llbb.ExTreeMaker."
    ...
    dataDirectory = str(os.environ["CMSSW_BASE"]) + "/src/cp3-llbb/ExTreeMaker/data/"
    ...
    toprint = ['runningMode', 'eventSelection', 'ptjet', 'btagging', 'WP', 'muChannel', 'eleChannel',
               'doMEcontrolPlots', 'doNNJetRegression']
    ...
    eventCollections = [eventCollection("genParticles", "vector<pat::PackedGenParticle>", "packedGenParticles"),
                        eventCollection("lheParticles", "LHEEventProduct", "source"),
                        eventCollection("genJets", "vector<reco::GenJet>", "slimmedGenJets"),
    ...
    eventProducers = [eventProducer("vertex", "ObjectSelection", "vertex", {}),
                      eventProducer("goodJets_mu", "ObjectSelection", "goodJets",
                                    {"muChannel": True, "eleChannel": False, "pt": ptjet}),
                      eventProducer("goodJets_ele", "ObjectSelection", "goodJets",
                                    {"muChannel": False, "eleChannel": True, "pt": ptjet}),
                      eventProducer("goodJets_all", "ObjectSelection", "goodJets",
                                    {"muChannel": True, "eleChannel": True, "pt": ptjet}),
                      ...
                      eventProducer("bestZmumuCandidate", "ObjectSelection", "findBestCandidate",
                                    {"muChannel": True, "eleChannel": False}),
                      eventProducer("bestZelelCandidate", "ObjectSelection", "findBestCandidate",
                                    {"muChannel": False, "eleChannel": True}),
                      eventProducer("bestZcandidate", "ObjectSelection", "findBestCandidate",
                      ...
                      eventProducer("electronsPair", "ObjectSelection", "diLeptonsPair",
                                    {"bestLeptonCand": "bestZelelcandidate"}),
                      # eventProducer("muelPair", "ObjectSelection", "diLeptonsPair", { "bestLeptonCand":"bestZmuelcandidate" } ),
                      eventProducer("leptonsPair", "ObjectSelection", "diLeptonsPair",
                                    {"bestLeptonCand": "bestZcandidate"}),
                      eventProducer("dijet_muChannel", "ObjectSelection", "findDijetPair",
                                    {"btagging": btagging, "WP": WP, "muChannel": True, "eleChannel": False}),
                      eventProducer("dijet_eleChannel", "ObjectSelection", "findDijetPair",
                                    {"btagging": btagging, "WP": WP, "muChannel": False, "eleChannel": True}),
                      eventProducer("dijet_all", "ObjectSelection", "findDijetPair",
                                    {"btagging": btagging, "WP": WP, "muChannel": True, "eleChannel": True}),
                      eventProducer("sortedGenJets", "MonteCarloSelection", "genjetCollectionsProducer",
                                    {"ptcut": 0, "etacut": 10}),
                      eventProducer("ptSortedLeptons", "ObjectSelection", "leptonsFromPV_ptSorted", {}),
                      eventProducer("ptSortedLeptons_DRll", "ObjectSelection",
                                    "leptonsFromPV_ptSorted_DRllVetoOnFirstTwo",
                                    {"DRll_cut": 0.3, "ptLeadLep": 17.0, "ptSubLeadLep": 8.0})
                      ]
...
def changeDiLeptCand(conf=None, names={"muonsPair": "bestZmumucandidate", "electronsPair": "bestZelelcandidate",
                                       "muelPair": "bestZmuelcandidate", "leptonsPair": "bestZcandidate"}):
    if conf is None: return
    for name in names:
...
def updateConfMC(c=configuration):
    ...
    c.toprint.extend(
        ["JERfactor", "JESfactor", "LeptonTnPfactor", "SF_uncert", "SF_running_mode", "btagperfData", "pileupData",
         "pileupMC"])
