...
def main(options):
    ...
    runAnalysis(path=options.path, outputname=options.outputname, levels=levels, Njobs=options.Njobs,
    ...
    for event in events:
        ...
        if configuration.runningMode == "plots":
            ...
            for cp in controlPlots[:1]:
                ...
                cp.setCategories(map(lambda c: EventSelection.isInCategory(c, event.category),
