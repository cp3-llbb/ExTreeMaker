...
def isInCategory(category, categoryTuple):
    if category < len(categories):
        ...
        return isInCategoryChannel(category % len(categories),
                                   categoryTuple[len(categoryTuple) / 3:2 * len(categoryTuple) / 3])
    else:
        return isInCategoryChannel(category % len(categories),
                                   categoryTuple[2 * len(categoryTuple) / 3:len(categoryTuple)])
...
def eventCategory(event, btagging="CSV", WP=["M", "L"], ZjetFilter="none"):
    ...
    return eventCategoryChannel(event, muChannel=True, eleChannel=False, btagging=btagging, WP=WP,
                                ZjetFilter=ZjetFilter) + \
           eventCategoryChannel(event, muChannel=False, eleChannel=True, btagging=btagging, WP=WP,
                                ZjetFilter=ZjetFilter) + \
           eventCategoryChannel(event, muChannel=False, eleChannel=False, btagging=btagging, WP=WP,
                                ZjetFilter=ZjetFilter)
...
def eventCategoryChannel(event, muChannel=True, eleChannel=True, btagging="CSV", WP=["M", "L"], ZjetFilter="none"):
    ...
    if not ZjetFilter == "none":
        ...
        if (MonteCarloSelection.isRecoZbEvent(event) and not MonteCarloSelection.isRecoZbbEvent(event)) and not (
            '1b' in ZjetFilter): return [-1]
        if (not MonteCarloSelection.isRecoZbbEvent(event) and not MonteCarloSelection.isRecoZbEvent(event)) and not (
            '0b' in ZjetFilter): return [-1]
    output = []
    ...
    if bestDiLeptcandidate is None:
        ...
        if (lept1.isElectron() and lept2.isMuon() and mass > Mll_cut) or (
                lept2.isElectron() and lept1.isMuon() and mass > Mll_cut):
    ...
    if dijet[0] is None or dijet[1] is None:
        ...
        # fixme: ReferenceError: attempt to access a null-pointer
