...
def setupPatMuons(process, runOnMC, MuonPt=8.):
    process.patMuons.pfMuonSource = cms.InputTag(
        "pfSelectedMuons")  # FIX: pfSelectedMuons is used instead of pfIsolatedMuons used in the ZH anlysis, reason no obvious need to use pre isolated muons -> isolation done after
    ...
    for HLTMu in singleMuPath + doubleMuPath + MuEGPath: getattr(process,
                                                                 'muonTriggerMatchHLTMuons' + HLTMu[6:-5].replace("_",
                                                                                                                  "")).src = cms.InputTag(
        'selectedMuonsWithIsolationData')
