...
def setupPatMets(process, runOnMC, makeNoPUMet):
    ...
    if runOnMC:
        process.PFMETSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewPFMEtSysShiftCorrParameters_2012runABCDvsNvtx_mc,
            )

        process.MVAMETNURSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewMVAMEtNURSysShiftCorrParameters_2012runABCDvsNvtx_mc,
        ...
        process.MVAMETURSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewMVAMEtURSysShiftCorrParameters_2012runABCDvsNvtx_mc,
            src=cms.InputTag('pfMEtMVA')
            )

        process.NoPUMETSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewNoPUMEtSysShiftCorrParameters_2012runABCDvsNvtx_mc,
            src=cms.InputTag('noPileUpPFMEt')
            )
    else:

        process.PFMETSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewPFMEtSysShiftCorrParameters_2012runABCDvsNvtx_data,
            )

        process.MVAMETNURSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewMVAMEtNURSysShiftCorrParameters_2012runABCDvsNvtx_data,
            src=cms.InputTag('pfMEtMVA')
            )

        process.MVAMETURSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewMVAMEtURSysShiftCorrParameters_2012runABCDvsNvtx_data,
            src=cms.InputTag('pfMEtMVA')
            )

        process.NoPUMETSysShiftCorr = process.pfMEtSysShiftCorr.clone(
            parameter=process.NewNoPUMEtSysShiftCorrParameters_2012runABCDvsNvtx_data,
            src=cms.InputTag('noPileUpPFMEt')
            )
    ...
    process.metUncertaintySequence.replace(process.patType1CorrectedPFMet,
                                           cms.Sequence(
                                               process.type0PFMEtCorrection * process.patPFMETtype0Corr * process.patType1CorrectedPFMet)
                                           )
