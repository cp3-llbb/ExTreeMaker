...
def setupPatElectrons(process, runOnMC, ElePt=8.):
    ...
    process.selectedElectronsWithIsolationData = cms.EDProducer(
        ...
        IsoValElectronNoPF=cms.VInputTag(cms.InputTag('elPFIsoValueCharged03NoPFIdPFIso'),
                                         # use PFIdPFIso or NoPFIdPFIso: second one make more sense but no eplicit documentation about it, both producer take the same inputs then they are probably just the same
    ...
    for HLTEle in singleElePath + doubleElePath + MuEGPath: getattr(process,
                                                                    'eleTriggerMatchHLT' + HLTEle[6:-5].replace("_",
                                                                                                                "")).src = cms.InputTag(
        "selectedElectronsWithIsolationData")
    ...
    process.zmvaelTightmvaelTight = cms.EDProducer('CandViewShallowCloneCombiner',
                                                   decay=cms.string(
                                                       'tightMVAElectronsNonTrig@+ tightMVAElectronsNonTrig@-'),
                                                   cut=cms.string('mass > 10.0'),
